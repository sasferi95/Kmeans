...
namespace kmeans
{
    class MainWindowLogic
    {
        ...
        static int[,] bestCentroidToPixels = new int[bitmapheight, bitmapwidth];
        static Dictionary<int, Color> centroidsWithSmallestDistance = new Dictionary<int, Color>();
        ...
        private Dictionary<int, Center> Clustering(int k, byte[] colours)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Dictionary<int, Center> centerInformaitons = new Dictionary<int, Center>();//centerek
                InitDictionary(colours, centerInformaitons, k, colours.Length);


                int[,] closestCentroid = new int[bitmapwidth, bitmapheight];

                //jelenleg fixen 20x léptetjük a középpontot
                for (int z = 0; z < 20; z++)
                {
                    ResetCentorids(centerInformaitons);
                    for (int idx = 0; idx < colours.Length / 4; idx++)
                    {
                        int oneColour = idx * 4;
                        //legközelebbi centroidot adja vissza
                        ...
                        int centroidId = closest.Key;
                        centerInformaitons[centroidId].SumRed += colours[oneColour + 2];
                        centerInformaitons[centroidId].SumGreen += colours[oneColour + 1];
                        centerInformaitons[centroidId].SumBlue += colours[oneColour];
                        centerInformaitons[centroidId].SumInGroup++;
                        centerInformaitons[centroidId].SumDistance += closest.Value;
        ...
        {
            for (int i = 0; i < centerInformaitons.Count; i++)
            {
